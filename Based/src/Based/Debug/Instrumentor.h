        ...
        public:
            ...
            void BeginSession(const std::string& name, const std::string& filepath = "results.json")
            {
                ...
                if (m_CurrentSession)
                {
                    ...
                    { // Edge case: BeginSession() might be before
                      // Log::Init()
                ...
                else
                {
                    if (Log::GetCoreLogger())
                    {
                        BSD_CORE_ERROR("Instrumentor could not open results file '{0}", filepath);
                    }
                }
            }
...
#define BSD_PROFILE 0
    ...
    #endif
    #define BSD_PROFILE_BEGIN_SESSION(name, filepath) ::Based::Instrumentor::Get().BeginSession(name, filepath)
    #define BSD_PROFILE_END_SESSION()                 ::Based::Instrumentor::Get().EndSession()
    #define BSD_PROFILE_SCOPE(name)                                                                                    \
        constexpr auto fixedName = ::Based::InstrumentorUtils::CCleanupOutputString(name, "__cdecl ");                 \
